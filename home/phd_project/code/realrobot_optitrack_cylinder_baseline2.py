...
class InitialSimulationModel():
    ...
    def initial_particle(self):
        ...
        real_object_x_ang = copy.deepcopy(self.cylinder_real_object_start_angle[0])
        real_object_y_ang = copy.deepcopy(self.cylinder_real_object_start_angle[1])
        real_object_z_ang = copy.deepcopy(self.cylinder_real_object_start_angle[2])
        real_object_ang = [real_object_x_ang,real_object_y_ang,real_object_z_ang]
        noise_object_x = self.add_noise_to_obs_model(self.cylinder_real_object_start_pos[0])
        noise_object_y = self.add_noise_to_obs_model(self.cylinder_real_object_start_pos[1])
        noise_object_z = self.cylinder_real_object_start_pos[2]
        noise_object = [noise_object_x,noise_object_y,noise_object_z]
        noise_object_x_ang = self.cylinder_real_object_start_angle[0]
        noise_object_y_ang = self.cylinder_real_object_start_angle[1]
...
class PFMove():
    ...
    def update_particle_filter_cheat(self, pybullet_sim_env, fake_robot_id, real_robot_joint_pos, observation, pw_T_object_ori):
        ...
        if self.u_flag >= 21:
            ...
            boss_error_df.to_csv('error_sum_0_5_50.csv',index=0,header=0,mode='a')
        ...
        if self.u_flag >= 21:
            print("write error file")
            boss_bsln2_df.to_csv('baselin2_error_sum_0_5_50.csv',index=0,header=0,mode='a')
    ...
    def observation_update(self, observation,pw_T_object_ori):
        ...
        if self.u_flag >= 21:
            ...
            boss_obser_df.to_csv('obser_sum_0_5_50.csv',index=0,header=0,mode='a')        
