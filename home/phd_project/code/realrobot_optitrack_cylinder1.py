...
class InitialSimulationModel():
    ...
    def initial_and_set_simulation_env(self,pos_of_robot):
        for index, particle in enumerate(self.particle_cloud):
            #input('Press [ENTER] to generate particle')
            ...
            while True:
                ...
                for t_i in range(length):
                    if collide_ids[t_i][1] == 8:
                        Px,Py,Pz = self.generate_random_pose()
...
class PFMove():
    ...
    def update_particle_filter_cheat(self, pybullet_sim_env, fake_robot_id, real_robot_joint_pos, observation, pw_T_object_ori):
        ...
        if self.u_flag >= 7:
            ...
            boss_error_df.to_csv('error_sum_0_1.csv',index=0,header=0,mode='a')
    ...
    def observation_update(self, observation,pw_T_object_ori):
        ...
        if self.u_flag >= 7:
            ...
            boss_obser_df.to_csv('obser_sum_0_1.csv',index=0,header=0,mode='a')        
...
if __name__ == '__main__':
    ...
    optitrack_object_id = p_visualisation.loadURDF(os.path.expanduser("~/phd_project/object/cylinder_real_object_with_visual_small.urdf"),pw_T_object_pos,pw_T_object_ori)                          
