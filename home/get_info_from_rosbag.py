...
info_dict = yaml.load(Bag(os.path.expanduser(f"~/rosbag/new_camera_cracker{rosbag_num}.bag"))._get_yaml_info(), Loader=yaml.FullLoader)
# info_dict = yaml.load(Bag(os.path.expanduser(f"~/rosbag/pointcloud_cracker2.bag"))._get_yaml_info(), Loader=yaml.FullLoader)

dura = (info_dict['duration'] + 1) * 12.5
