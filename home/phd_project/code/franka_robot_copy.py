...
class InitialSimulationModel():
    ...
    def initial_particle(self):
        for i in range(self.particle_num):
            x = random.uniform(self.cylinder_real_object_start_pos[0] - 0.1, self.cylinder_real_object_start_pos[0] + 0.1)
            y = random.uniform(self.cylinder_real_object_start_pos[1] - 0.1, self.cylinder_real_object_start_pos[1] + 0.1)
            z = self.cylinder_real_object_start_pos[2]
            w = 1/self.particle_num
            particle = Particle(x,y,z,w,index=i)
            self.particle_cloud.append(particle)
        object_estimate_pos_x,object_estimate_pos_y = self.compute_estimate_pos_of_object(self.particle_cloud)
...
class PFMove():
    ...
    #new structure
    def real_robot_control(self,observation):   
    ...
    #executed_control 
    def update_particle_filter_cheat(self, pybullet_sim_env, fake_robot_id, real_robot_joint_pos, observation):
...
if __name__ == '__main__':
    ...
    #get real object pos
    real_object_last_update_pos = get_real_object_pos(real_object_id)
    ...
    while True:
        ...
        real_object_current_pos = get_real_object_pos(real_object_id)#Cheat
        ...
        if distance_between_current_and_old > d_thresh_limitation:
            ...
            #Cheat
            observation = get_observation(real_object_id) #get pos of real object
