...
title_ADD = object_name+": "+ang_and_pos+" Matrix errors (m) vs Time (s)"
...
if ang_and_pos == "ADD" or ang_and_pos == "ADDS" :
    if sceneName == "scene1":
        x_range_max = 1000 # 28, 129, 265
        x_range_unit = 100 # 2, 6, 25, 125
        y_range_max = 0.5 # 0.5
    ...
    if ang_and_pos == "ADDS":
        dataset_ADD.columns=["index","time","alg","obj_scene","particle_num","ray_type","obj_name","ADDS Matrix Error (m)"]
    print(pd.__version__)
    print(sns.__version__)
    print(matplotlib.__version__)
    color_map = {
    ...
    # print(dataset_ADD)
    if ang_and_pos == "ADD":
        figure_ADD = sns.lineplot(data=dataset_ADD, x="time", y="ADD Matrix Error (m)", hue='alg', errorbar=('ci', 95), legend=True, linewidth=0.5, palette=color_map)
    if ang_and_pos == "ADDS":
        figure_ADD = sns.lineplot(data=dataset_ADD, x="time", y="ADDS Matrix Error (m)", hue='alg', errorbar=('ci', 95), legend=True, linewidth=0.5, palette=color_map)
    ...
    svg_fig_ADD.savefig(save_file_path+file_name+"_par_avg.png",format="png")
    # svg_fig_ADD.savefig(save_file_path+file_name+".svg", format="svg", dpi=150)

    print("finished par avg "+ang_and_pos+" plot")