...
p_visualisation.resetDebugVisualizerCamera(cameraDistance=1, cameraYaw=180, cameraPitch=-85,
...
class Ros_listener():
    def __init__(self):
        ...
        self.robot_pose = rospy.Subscriber('/mocap/rigid_bodies/pandaRobot/pose', PoseStamped, self.robot_pose_callback,
                                           queue_size=10)
        self.object_pose = rospy.Subscriber('/mocap/rigid_bodies/cheezit/pose', PoseStamped, self.object_pose_callback,
                                            queue_size=10)
...
class InitialRealworldModel():
    ...
    def initial_robot(self, robot_pos, robot_orientation=[0, 0, 0, 1]):
        ...
        real_robot_id = p_visualisation.loadURDF(os.path.expanduser(
            "~/phd_project/data/bullet3-master/examples/pybullet/gym/pybullet_data/franka_panda/panda.urdf"),
                                                 robot_pos,
                                                 robot_orientation,
                                                 useFixedBase=1)
    ...
    def initial_target_object(self, object_pos, object_orientation=[0, 0, 0, 1]):
        ...
        real_object_id = p_visualisation.loadURDF(
            os.path.expanduser("~/phd_project/object/cube/cheezit_obj_small_hor.urdf"),
            object_pos,
            object_orientation)
...
class InitialSimulationModel():
    ...
    def initial_particle(self):
        ...
        self.noise_object_pose = [noise_obj_x, noise_obj_y, noise_obj_z, noise_obj_x_ang, noise_obj_y_ang,
                                  noise_obj_z_ang]

        for i in range(self.particle_num):
            x, y, z, x_angle, y_angle, z_angle, new_quat = self.generate_random_pose(self.noise_object_pose,
                                                                                     self.pw_T_object_ori_dope)
    ...
    def display_particle(self):
        for index, particle in enumerate(self.particle_cloud):
            ...
            visualize_particle_Id = p_visualisation.loadURDF(
                os.path.expanduser("~/phd_project/object/cube/cheezit_par_with_visual_small_PE_hor.urdf"),
    ...
    def display_particle_PM(self):
        for index, particle in enumerate(self.particle_cloud_PM):
            ...
            visualize_particle_orientation = p_visualisation.getQuaternionFromEuler(visualize_particle_angle)
            visualize_particle_Id = p_visualisation.loadURDF(
                os.path.expanduser("~/phd_project/object/cube/cheezit_par_with_visual_small_PM_hor.urdf"),
                visualize_particle_pos,
                visualize_particle_orientation)
    ...
    def initial_and_set_simulation_env(self, joint_of_robot):
        for index, particle in enumerate(self.particle_cloud):
            ...
            fake_robot_id = pybullet_simulation_env.loadURDF(os.path.expanduser(
                "~/phd_project/data/bullet3-master/examples/pybullet/gym/pybullet_data/franka_panda/panda.urdf"),
            ...
            particle_no_visual_start_orientation = pybullet_simulation_env.getQuaternionFromEuler(
                particle_no_visual_start_angle)
            particle_no_visual_id = pybullet_simulation_env.loadURDF(
                os.path.expanduser("~/phd_project/object/cube/cheezit_par_no_visual_small_hor.urdf"),
            ...
            while True:
                ...
                for t_i in range(length):
                    if collide_ids[t_i][1] == 8:
                        Px, Py, Pz, Px_angle, Py_angle, Pz_angle, P_quat = self.generate_random_pose(
                            self.noise_object_pose, self.pw_T_object_ori_dope)
                        particle_no_visual_angle = [Px_angle, Py_angle, Pz_angle]
                        particle_no_visual_ori = pybullet_simulation_env.getQuaternionFromEuler(
                            particle_no_visual_angle)
    ...
    def initial_and_set_simulation_env_PM(self, joint_of_robot):
        ...
        return obj_est_set_PM[0], obj_est_set_PM[1], obj_est_set_PM[2], obj_est_set_PM[3], obj_est_set_PM[4], \
               obj_est_set_PM[5]
...
class PFMove():
    ...
    # new structure
    def real_robot_control_PE(self, opti_obj_pos_cur, opti_obj_ori_cur, real_robot_joint_pos, nois_obj_pos_cur,
                              nois_obj_ang_cur):
        # Cheat
    ...
    def update_particle_filter_PE(self, pybullet_sim_env, fake_robot_id, real_robot_joint_pos, opti_obj_pos_cur,
                                  opti_obj_ori_cur, nois_obj_pos_cur, nois_obj_ang_cur):
        ...
        estimated_object_pos, estimated_object_ang = self.observation_update_PE(opti_obj_pos_cur, opti_obj_ori_cur,
                                                                                nois_obj_pos_cur, nois_obj_ang_cur)
    ...
    def motion_update_PE_parallelised(self, pybullet_sim_env, fake_robot_id, real_robot_joint_pos):
        ...
        start = time.time()
        for index, pybullet_env in enumerate(pybullet_sim_env):
            thread = threading.Thread(target=self.function_to_parallelise,
                                      args=(index, pybullet_env, fake_robot_id, real_robot_joint_pos))
            thread.start()
    ...
    def function_to_parallelise(self, index, pybullet_env, fake_robot_id, real_robot_joint_pos):
        ...
        ### ori: x,y,z,w
        sim_par_cur_pos, sim_par_cur_ori = self.get_item_pos(pybullet_env,
                                                             initial_parameter.particle_no_visual_id_collection[index])
    ...
    def set_paticle_in_each_sim_env(self):
        for index, pybullet_env in enumerate(self.pybullet_env_id_collection):
            visual_particle_pos = [self.particle_cloud[index].x, self.particle_cloud[index].y,
                                   self.particle_cloud[index].z]
            visual_particle_angle = [self.particle_cloud[index].x_angle, self.particle_cloud[index].y_angle,
...
class PFMovePM():
    def __init__(self, robot_id=None, real_robot_id=None, object_id=None):
        ...
        self.particle_with_visual_id_collection_PM = copy.deepcopy(
            initial_parameter.particle_with_visual_id_collection_PM)
    ...
    def update_particle_filter_PM(self, opti_obj_pos_cur, opti_obj_ori_cur, nois_obj_pos_cur, nois_obj_ang_cur):
        ...
        t2 = time.time()
        estimated_object_pose_PM = self.observation_update_PM(opti_obj_pos_cur, opti_obj_ori_cur, nois_obj_pos_cur,
                                                              nois_obj_ang_cur)
        estimated_object_pos_PM = [estimated_object_pose_PM[0], estimated_object_pose_PM[1],
                                   estimated_object_pose_PM[2]]
        estimated_object_ang_PM = [estimated_object_pose_PM[3], estimated_object_pose_PM[4],
                                   estimated_object_pose_PM[5]]
    ...
    def set_paticle_in_each_sim_env_PM(self):
        for index, pybullet_env in enumerate(self.pybullet_env_id_collection_PM):
            visual_particle_pos = [self.particle_cloud_PM[index].x, self.particle_cloud_PM[index].y,
                                   self.particle_cloud_PM[index].z]
            visual_particle_angle = [self.particle_cloud_PM[index].x_angle, self.particle_cloud_PM[index].y_angle,
                                     self.particle_cloud_PM[index].z_angle]
            visual_particle_orientation = pybullet_env.getQuaternionFromEuler(visual_particle_angle)
...
if __name__ == '__main__':
    ...
    if visualisation_flag == True:
        optitrack_object_id = p_visualisation.loadURDF(
            os.path.expanduser("~/phd_project/object/cube/cheezit_real_obj_with_visual_small_hor.urdf"),
            pw_T_object_pos,
            pw_T_object_ori)
    ...
    if visualisation_flag == True:
        dope_object_id = p_visualisation.loadURDF(
            os.path.expanduser("~/phd_project/object/cube/cheezit_dope_obj_with_visual_small_PE_hor.urdf"),
    ...
    initial_parameter = InitialSimulationModel(particle_num, pybullet_robot_pos, pybullet_robot_ori, dope_obj_pos_init,
                                               dope_obj_ang_init, dope_obj_ori_init)
    ...
    if visualisation_flag == True:
        estimated_object_id = p_visualisation.loadURDF(
            os.path.expanduser("~/phd_project/object/cube/cheezit_est_obj_with_visual_small_PE_hor.urdf"),
            estimated_object_pos,
            estimated_object_ori)
        estimated_object_id_PM = p_visualisation.loadURDF(
            os.path.expanduser("~/phd_project/object/cube/cheezit_est_obj_with_visual_small_PM_hor.urdf"),
            estimated_object_pos,
            estimated_object_ori)
    # input('test')
    # compute error
